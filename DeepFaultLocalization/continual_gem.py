...
def _train_incremental_with_replay_agem(
    sess,
    graph,
    train_x,
    train_y,
    ...
    try:
        for _ in range(epochs):
            ...
            for __ in range(total_batch):
                ...
                before_logits = cr._predict_logits(sess, graph, bx)
                fd_cur = cr._train_feed_dict(graph, bx, by, bg, cst.DROPOUT_RATE)

                # A-GEM hard constraint: project current gradient by replay reference gradient.
                if use_replay and replay_buffer is not None and replay_buffer.size > 0:
                    ...
                    if rb_ref is not None:
                        rx_ref, ry_ref, rg_ref, _ = rb_ref
                        fd_ref = cr._train_feed_dict(graph, rx_ref, ry_ref, rg_ref, cst.DROPOUT_RATE)
                        ...
                        if g_cur.shape[0] > 0 and g_ref.shape[0] == g_cur.shape[0]:
                            ...
                            # Fallback for edge cases with empty grad list.
                            if use_ewc and ewc_state is not None and ewc_stats is not None:
                                fd_fallback = dict(fd_cur)
                                fd_fallback.update(cst._ewc_feed(ewc_state, ewc_stats["omega"], ewc_stats["star"]))
                                sess.run(ewc_state["optimizer_ewc"], feed_dict=fd_fallback)
                            else:
                ...
                if use_replay and replay_buffer is not None and replay_buffer.size > 0:
                    for _j in range(int(max(1, replay_per_step))):
                        ...
                        if replay_beta > 0.0 or distill_alpha > 0.0:
                            fd_rep = cr._train_feed_dict(graph, rx, ry, rg, cst.DROPOUT_RATE)
...
def main():
    pa = argparse.ArgumentParser(
        ...
        description="流式持续学习 + replay + DER++ + A-GEM（增量阶段硬启用 GEM）。",
    )
    # Keep args consistent with continual_replay.py
    ...
    pa.add_argument(
        "--out_dir",
        default=None,
        help="输出根目录；省略则为 result_continual/<model>/<subject>/..._g1/",
    )
    ...
    if args.model not in supported:
        print("continual_gem: model 必须是: %s" % " ".join(supported), file=sys.stderr)
        sys.exit(1)
    if args.loss != "softmax":
        print("continual_gem: 当前仅支持 loss=softmax", file=sys.stderr)
        sys.exit(1)
    if args.tech != "DeepFL_CL":
        print("continual_gem: 为避免数据协议泄漏，tech 仅允许 DeepFL_CL", file=sys.stderr)
        sys.exit(1)
    ...
    run_tag = "ur%d_ue%d_lr%.4f_rps%d_da%.2f_el%.1f_eg%.2f_g1_wup%d_te%d_incr%d" % (
    ...
    agem_state = _build_agem_ops(graph)
    ...
    replay_buffer = cr.ReplayBuffer(args.replay_size, n_input) if args.use_replay else None
    ...
    lines = [
        ...
        "use_replay=%s use_ewc=%s use_gem=True replay_size=%d replay_per_step=%d replay_beta=%.3f distill_alpha=%.3f temperature=%.3f ewc_lambda=%.3f ewc_gamma=%.3f"
        % (
            str(args.use_replay),
            str(args.use_ewc),
