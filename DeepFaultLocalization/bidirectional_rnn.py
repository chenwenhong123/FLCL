...
def BiRNN(x, weights, biases, n_hidden, n_steps, keep_prob):
    ...
    lstm_fw_cell = DropoutWrapper(lstm_fw_cell, output_keep_prob=keep_prob)
    # Backward direction cell
    lstm_bw_cell = BasicLSTMCell(n_hidden, forget_bias=1.0)
    lstm_bw_cell = DropoutWrapper(lstm_bw_cell, output_keep_prob=keep_prob)
...
def run(trainFile, trainLabelFile, testFile, testLabelFile, groupFile, suspFile, featureDistribution, loss):
    ...
    regularizer = tf.add_n([tf.nn.l2_loss(v) for v in variables if 'bias' not in v.name]) * L2_value if use_l2 else tf.constant(0.0, dtype=tf.float32)
    cost = ut.loss_func(pred, y, loss, datasets, g)
    optimizer = create_optimizer().minimize(cost + regularizer)
