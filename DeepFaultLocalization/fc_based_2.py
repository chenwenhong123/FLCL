...
def create_variables(name, shape, initializer=None):
    # tf.contrib was removed in TF2; use TF2-compatible equivalents.
    if initializer is None:
        initializer = tf.glorot_uniform_initializer()
    regularizer = tf.keras.regularizers.l2(L2_value) if use_l2 else None
...
def run(trainFile, trainLabelFile, testFile,testLabelFile, groupFile, suspFile,loss, featureNum, nodeNum):
    ...
    with tf.control_dependencies(update_ops):
        reg_term = tf.add_n(regu_losses) if (use_l2 and regu_losses) else tf.constant(0.0, dtype=tf.float32)
        optimizer = create_optimizer().minimize(cost + reg_term)
