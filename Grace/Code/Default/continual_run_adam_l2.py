...
def _build_adam_optimizer(model: NlEncoder, lr: float, weight_decay: float, beta1: float, beta2: float):
    return ScheduledOptim(
        optim.Adam(model.parameters(), lr=lr, betas=(beta1, beta2), weight_decay=weight_decay),
        32,
        4000,
    )
...
def main():
    if len(sys.argv) < 5:
        print(
            "Usage: python continual_run_adam_l2.py <Project> <lr> <seed> <batch_size> "
            "[warmup_ids=10] [base_epochs=15] [inc_epochs=2] [weight_decay=1e-4] [beta1=0.9] [beta2=0.999]"
        )
        sys.exit(1)
    ...
    print(f"optimizer: Adam(betas=({beta1}, {beta2}), weight_decay={weight_decay})")
    ...
    for bid in tqdm(warmup_range, desc="Warmup Base Selection", ncols=100):
        ...
        optimizer = _build_adam_optimizer(
            model, lr=args.lr, weight_decay=weight_decay, beta1=beta1, beta2=beta2
        )
        train_on_one_id(
    ...
    optimizer = _build_adam_optimizer(model, lr=args.lr, weight_decay=weight_decay, beta1=beta1, beta2=beta2)
