...
class GCNN(nn.Module):
    ...
    def forward(self, state, left, inputad):
        ...
        state = self.subconnect(
            state,
            lambda _x: self.lstm(
                torch.bmm(degree2, state).reshape(-1, self.hiddensize),
                (
                    torch.zeros(_x.reshape(-1, self.hiddensize).size(), device=_x.device),
                    _x.reshape(-1, self.hiddensize),
                ),
            )[1].reshape(-1, s, self.hiddensize),
        )  # state + torch.matmul(degree2, state)
