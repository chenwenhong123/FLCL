...
def train_incremental_with_replay_ewc(
    model: NlEncoder,
    ...
    for _ in range(inc_epochs):
        ...
        # EWC 正则（在线版）
        if ewc_lambda > 0:
            ewc_pen = compute_ewc_penalty(model, prev_params, fisher)
            total_loss = total_loss + ewc_lambda * ewc_pen
...
def main():
    if len(sys.argv) < 5:
        print(
            "Usage: python continual_run_ewc.py <Project> <lr> <seed> <batch_size> "
    ...
    run_id = (
        f"ewc_base{base_epochs}_inc{inc_epochs}_lr{lr}_bs{batch_size}_warm{warmup_ids}"
    ...
    rows = []
    for t in tqdm(stream_range, desc="Incremental Stream (Replay+DER++EWC)", ncols=100):
        train_incremental_with_replay_ewc(
            model=model,
            optimizer=optimizer,
            args=args,
            project=project,
